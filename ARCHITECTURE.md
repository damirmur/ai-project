...
My Assistant AI — приложение на Node.js/TypeScript для тестирования локальных LLM моделей с использованием `node-llama-cpp`. Приложение следует модульной сервис-ориентированной архитектуре с чётким разделением ответственности, автоматическим расчётом размера контекста на основе доступной памяти, поддержкой function calling (инструментов) и автоматическим тест-раннером для циклического тестирования.
...
- **Function Calling**: Поддержка инструментов (tools) для вызова функций моделью
...
- Вопросы, триггерящие использование инструментов (tools)
...
5. "Какая сейчас погода в Москве? Используй инструмент для проверки погоды."
6. "Сколько будет 245 умножить на 37? Используй калькулятор."
7. "Какая столица у Франции? Найди в базе знаний."
8. "Посчитай 100 + 250 и скажи результат"
...
### 4. Слой инструментов (`src/tools/`)

Модуль function calling — позволяет модели вызывать predefined функции для получения конкретной информации.

#### `index.ts` — Определение инструментов

Содержит три инструмента, определённых через `defineChatSessionFunction`:

**`getWeather`** — Получение погоды
- Параметры: `city` (string) — название города
- Возвращает: температуру в формате `{ city, temperature, status }`
- Доступные города: Moscow (15°C), London (12°C), Paris (18°C), Tokyo (25°C), New York (20°C)

**`calculator`** — Калькулятор
- Параметры: `expression` (string) — математическое выражение
- Возвращает: результат вычисления `{ expression, result, status }`
- Поддерживаемые операторы: `+`, `-`, `*`, `/`, скобки `()`, десятичные числа
- Валидация входных данных через regex

**`searchKnowledge`** — Поиск знаний
- Параметры: `query` (string) — поисковый запрос
- Возвращает: найденную информацию `{ query, result, status }`
- База знаний: capital of russia → Moscow, capital of france → Paris, capital of japan → Tokyo

**Экспорт**:
```typescript
export const availableFunctions = { getWeather, calculator, searchKnowledge };
...
**Функции**:
```typescript
logToolCall(name: string, params: Record<string, unknown>): void
logToolResult(name: string, result: unknown): void
logToolError(name: string, error: unknown): void
```
...
🔧 [TOOL RESULT] getWeather
   Результат: {"city": "Moscow", "temperature": "15°C", "status": "found"}
```

### 5. Сервис обнаружения GPU (`src/services/detect-gpu.service.ts`)

**Ответственность**:
- Автоматическое определение наличия GPU в системе
- Проверка наличия NVIDIA GPU через `nvidia-smi`
- Проверка наличия Vulkan SDK через `which glslc`
- Предоставление информации о GPU для оптимизации сборки

**Методы**:
```typescript
detectGPU(): Promise<{ type: 'cuda' | 'vulkan' | 'cpu' } | null>
```

### 6. Точка входа (`src/index.ts`)
...
### 7. Тест-раннер (`src/test-runner.ts`)
...
```
Запрос модели → LLMService → availableFunctions → handler → Результат → Модель
      ↓
logToolCall/logToolResult → Консоль
```
...
### Сборка
- **build:cpu**: Сборка node-llama-cpp для CPU (release b8771)
- **build:cuda**: Сборка node-llama-cpp для CUDA GPU (release b8771)
- **build:vulkan**: Сборка node-llama-cpp для Vulkan GPU (release b8771)
- **build:auto**: Автоматическое определение GPU и выбор типа сборки
...
3. **Новые инструменты**: Добавить в `src/tools/index.ts` через `defineChatSessionFunction()`
4. **Сбор метрик**: Расширить интерфейс `ITestResult`
5. **Дополнительные сервисы**: Реализовать интерфейсы сервисов
6. **Интеграция с БД**: Добавить новый модуль сервиса
7. **API слой**: Добавить HTTP/REST сервис
8. **Параллельное тестирование**: Запускать модели одновременно в test-runner
9. **Сравнение моделей**: Добавить аналитику сравнения метрик
10. **Расширение базы знаний**: Добавить больше данных в `searchKnowledge`
11. **Внешние API**: Интегрировать реальные API для инструментов (погода, калькулятор, поиск)
...
- ✅ Реализована поддержка function calling (инструменты/tools)
- ✅ Добавлены инструменты: getWeather, calculator, searchKnowledge
- ✅ Создан сервис обнаружения GPU (detect-gpu.service.ts)
- ✅ Добавлена поддержка Vulkan GPU сборки
- ✅ Создана утилита логирования инструментов (tools/logger.ts)
